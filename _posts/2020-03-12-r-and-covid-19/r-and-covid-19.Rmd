...
description: |
  Modelling the epidemic in European countries.  
author:
...
(cumconfplot <- countries %>% 
   ...
   labs(
     ...
     caption = glue("Data last updated on {format(Sys.Date()-1, '%B %d %Y')}"))
)
...
joined %>% 
  filter(country %in% countries_list, 
         type == "confirmed", 
         class == "cases") %>% 
  ggplot(aes(date, value, fill = dataset)) + 
...
China is actually doing pretty well, as the number of new confirmed cases has dropped down dramatically since March 3. Once they implemented the movement restrictions, the number of new cases has dropped down to a trickle, even if the total number of cases is still high (not shown). 
...
SIR_model <- function(country, data, N, forward = 0){
...
countries.nest <- countries.nest %>% 
  mutate(
    sirmodel = pmap(list(country, data, N, f = 7), SIR_model)
  ) 
    ...
    sirplot = map2(
      ...
      ~ggplot(data = .x, aes(date))+
        ...
        labs(y = "Cumulative incidence",
             ...
             caption = "(red=fitted incidence from SIR model, orange=observed incidence. Forecast for the next week)"
        )
    )
  )


setNames(countries.nest$sirplot, countries.nest$country)
