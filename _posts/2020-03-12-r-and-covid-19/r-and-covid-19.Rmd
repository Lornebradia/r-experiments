...
Most importantly, we want to model not just a single country, but a list of them; it would be of course highly inefficient to basically repeat the same lines of code over and over, replacing the country name at every step. So we need to wrap the modelling step in a function. Then we will apply this function via `purrr::map()` to a nested dataset. 
